...
class PixelSnail(nn.Module):
    def __init__(self, shape, nb_class, channel, kernel_size, nb_pixel_block, nb_res_block, res_channel, dropout=0.1, nb_cond_res_block=0, cond_res_channel=0, cond_res_kernel=3, nb_out_res_block=0, cond_interpolate=1, attention=True):
        ...
        self.cond_interpolate = cond_interpolate
        ...
        if nb_cond_res_block > 0:
            ...
            cond_net.extend([GatedResBlock(cond_res_channel, cond_res_channel, cond_res_kernel, dropout=dropout) for _ in range(nb_cond_res_block)])
            self.cond_net = nn.Sequential(*cond_net)

        out = []
        for _ in range(nb_out_res_block):
            out.append(GatedResBlock(channel, res_channel, 1, dropout=dropout))
    ...
    def forward(self, x, c=None, cache=None):
        ...
        if c != None:
            if 'c' in cache:
                ...
                c = self.cond_net(c)
                c = F.interpolate(c, scale_factor=self.cond_interpolate) # TODO: Could have some tranposed Conv2d instead
